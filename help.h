...
void myLine(int x1, int y1, int x2, int y2, int c1, int c2, int c3) //use three 3 integers if you want colored lines.
{
	HWND console_handle = GetConsoleWindow();
	HDC device_context = GetDC(console_handle);

	//change the color by changing the values in RGB (from 0-255) to get shades of gray. For other colors use 3 integers for colors.
	HPEN pen = CreatePen(PS_SOLID, 2, RGB(c1, c2, c3)); //2 is the width of the pen
...
void myEllipse(int x1, int y1, int x2, int y2, int R, int G, int B, int R1, int R2, int R3)
{
	HWND console_handle = GetConsoleWindow();
	HDC device_context = GetDC(console_handle);
	...
	HPEN pen = CreatePen(PS_SOLID, 2, RGB(R, G, B));
	SelectObject(device_context, pen);
	HBRUSH brush = ::CreateSolidBrush(RGB(R1, R2, R3));  //Fill color is black
	SelectObject(device_context, brush);
